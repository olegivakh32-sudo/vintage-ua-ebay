...
def _refresh_google_access_token_if_possible():
    refresh = session.get("google_refresh_token")
    if not refresh:
        return False
    try:
...
def _refresh_picker_media_urls(picker_id):
    """Refresh short-lived Google Photos baseUrl values without changing selection/order."""
    fresh, err = get_selected_items()
    if err == "GOOGLE_AUTH_EXPIRED" and _refresh_google_access_token_if_possible():
        fresh, err = get_selected_items()
    if err or not fresh:
        return False
    old = PHOTO_CACHE.get(picker_id, [])
    def ident(x):
        mf=x.get("mediaFile") or {}
        return (str(x.get("id") or ""), str(mf.get("filename") or ""))
    if old and [ident(x) for x in old] != [ident(x) for x in fresh]:
        return False
    PHOTO_CACHE[picker_id] = fresh
    return True
...
def _download_picker_photo(index, edited=False, max_side=2400):
    ...
    for attempt in range(3):
        ...
        if index < 1 or index > len(items):
            raise ValueError("Photo cache expired or photo not found")
        base_url = (items[index - 1].get("mediaFile") or {}).get("baseUrl")
        if not base_url:
            raise ValueError("Google Photos baseUrl missing")
        r = requests.get(base_url + f"=w{max_side}-h{max_side}", headers={"Authorization": f"Bearer {token}"} if token else {}, timeout=60)
        if r.ok:
            data = r.content
            if edited:
            ...
            return data
        last = RuntimeError(f"Google photo HTTP {r.status_code}")
        # baseUrl is short-lived. Refresh the SAME Picker selection and retry.
        if r.status_code in (401, 403, 404):
            if r.status_code == 401:
                _refresh_google_access_token_if_possible()
            if _refresh_picker_media_urls(picker_id):
                continue
        break
    raise last or RuntimeError("Google photo download failed")
...
def approved_photo_to_data_url(picker_id, index, size=1024):
    """Use seller-approved frozen JPEG bytes for all downstream AI work.
    This removes dependence on short-lived Google Photos baseUrl values after approval.
    """
    snap = (APPROVED_PHOTO_CACHE.get(picker_id) or {}).get(int(index))
    if not snap or not snap.get("bytes"):
        raise RuntimeError(f"Approved photo {index} snapshot unavailable")
    data = snap["bytes"]
    try:
        from PIL import Image as _Img
        im = _Img.open(BytesIO(data)).convert("RGB")
        if size and max(im.size) > int(size):
            im.thumbnail((int(size), int(size)), _Img.Resampling.LANCZOS)
...
{{"scope":"ONE_PHYSICAL_ITEM|MULTIPLE_PHYSICAL_ITEMS|UNCERTAIN","confidence":0,"note":"short Ukrainian explanation"}}"""
    ...
    for i in idxs:
        try:
            content.append({"type":"input_image","image_url":approved_photo_to_data_url(picker_id, i, 640),"detail":"low"})
        except TypeError:
            content.append({"type":"input_image","image_url":approved_photo_to_data_url(picker_id, i, 640),"detail":"low"})
        except Exception:
            pass
...
def confirm_item_scope():
    ...
    if scope == "ONE_PHYSICAL_ITEM":
        ...
        # Continue inside the SAME Flask request. A 307 redirect was causing intermittent
        # Render/Internal Server Error on mobile POST replay.
...
def fast_research():
    ...
    analysis = request.form.get("analysis", "") or ANALYSIS_CACHE.get(picker_id, "")
    if not analysis:
        return page("Missing analysis", "<h2>AI analysis missing</h2><p><a href='/picker/items'>Return to approved photos</a></p>"), 400
    ...
    # Pre-research scope guard. A seller confirmation is authoritative for this lot.
    # IMPORTANT: never call the AI classifier again after the seller has confirmed ONE/MULTIPLE;
    # otherwise the classifier can overwrite the seller choice and create a confirmation loop.
    structure = ITEM_STRUCTURE_CACHE.get(picker_id) or {}
    if structure.get("source") != "SELLER":
        try:
...
The four prices must be numeric USD whenever listing is reasonably possible."""}]
    ...
    try:
        ...
        return page("Research temporarily unavailable", f"<h2>{label} — Research did not complete</h2><p>Your approved photos and ONE ITEM confirmation are preserved.</p><details><summary>Technical detail</summary><pre style='white-space:pre-wrap'>{html.escape(str(exc))}</pre></details><form action='/picker/fast-research' method='post'>{hidden_field('analysis',analysis)}<button type='submit'>Retry Research</button></form><p><a href='/picker/items'>Back to approved photos</a></p>"), 503
...
def estimate_item_size_weight_from_photos(picker_id=None, items=None, google_token=None):
    ...
    for _idx, item in enumerate(items, start=1):
        try:
            content.append({"type": "input_image", "image_url": approved_photo_to_data_url(picker_id, _idx, 1024), "detail": "high"})
            image_count += 1
        except Exception:
            pass
    if image_count == 0:
