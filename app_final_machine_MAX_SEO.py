...
{{"scope":"ONE_PHYSICAL_ITEM|MULTIPLE_PHYSICAL_ITEMS|UNCERTAIN","confidence":0,"note":"short Ukrainian explanation"}}"""
    ...
    if scope == "MULTIPLE_PHYSICAL_ITEMS" and conf < 70:
        scope = "UNCERTAIN"
    result={"scope":scope,"confidence":conf,"note":str(obj.get("note") or "").strip(),"source":"AI"}
...
def confirm_item_scope():
    picker_id = session.get("picker_session_id")
    items = PHOTO_CACHE.get(picker_id, [])
    analysis = request.form.get("analysis", "") or ANALYSIS_CACHE.get(picker_id, "")
    ...
    scope = str(request.form.get("scope") or "").strip().upper()
    if scope == "ONE_PHYSICAL_ITEM":
        ITEM_STRUCTURE_CACHE[picker_id] = {"scope":"ONE_PHYSICAL_ITEM","confidence":100,"note":"Підтверджено продавцем: усі фото показують один фізичний предмет з різних боків/деталей.","source":"SELLER"}
        # Clear only research outputs. Keep photo selection, edited/approved bytes, main photo and order.
        IDENTIFICATION_CACHE.pop(picker_id, None); MARKET_CACHE.pop(picker_id, None)
        return redirect(url_for("fast_research_confirmed"))
    if scope == "MULTIPLE_PHYSICAL_ITEMS":
        ITEM_STRUCTURE_CACHE[picker_id] = {"scope":"MULTIPLE_PHYSICAL_ITEMS","confidence":100,"note":"Підтверджено продавцем: фото містять кілька фізичних предметів.","source":"SELLER"}
        IDENTIFICATION_CACHE.pop(picker_id, None); MARKET_CACHE.pop(picker_id, None)
        return redirect('/picker/items')
    return page("Invalid confirmation", "<h2>Choose ONE ITEM or MULTIPLE ITEMS.</h2>"), 400
...
def fast_research_confirmed():
    ...
    if structure.get("scope") != "ONE_PHYSICAL_ITEM" or not analysis:
        return redirect('/picker/items')
    ...
    <p style='color:green'><b>✓ ONE physical item. Approved photos preserved.</b></p>
...
def rerun_research():
    ...
    if not analysis:
        return redirect('/picker/items')
    return redirect(url_for("fast_research_confirmed")) if (ITEM_STRUCTURE_CACHE.get(picker_id) or {}).get("scope") == "ONE_PHYSICAL_ITEM" else redirect('/picker/items')
...
def fast_research():
    ...
    if scope != "ONE_PHYSICAL_ITEM" or scope_conf < 70:
        ...
        # Do not throw the seller back to photo selection. Keep all approved photos and ask one fact only.
        return page(label+" — Confirm item count", f"""<h2>{label} — One quick confirmation</h2>
        <p><b>AI physical-item check:</b> {html.escape(scope)} ({scope_conf}%)</p>
        <p>{msg}</p>
        <p><b>No market research has run yet.</b> Your 1–24 selected and approved photos stay intact.</p>
        <p>How many physical sale items do these photos show?</p>
        <form action='/picker/confirm-item-scope' method='post' style='display:inline-block;margin:6px 8px 6px 0'>
          {hidden_field('analysis',analysis)}<input type='hidden' name='scope' value='ONE_PHYSICAL_ITEM'>
          <button type='submit' style='font-size:18px;padding:13px 18px;font-weight:bold'>ONE ITEM — continue research</button>
        </form>
        <form action='/picker/confirm-item-scope' method='post' style='display:inline-block;margin:6px 0'>
          {hidden_field('analysis',analysis)}<input type='hidden' name='scope' value='MULTIPLE_PHYSICAL_ITEMS'>
          <button type='submit' style='font-size:18px;padding:13px 18px'>MULTIPLE ITEMS — review photos</button>
        </form>
        <p><small>Seller confirmation overrides the uncertain AI count for this lot only.</small></p>"""), 409
...
CRITICAL PHYSICAL-COUNT RULE: classifier says ONE_PHYSICAL_ITEM. Treat ALL selected photos as views/details/front/back/attached or removable parts of ONE physical object. Never describe it as two items, a pair, a set, two pieces, or "lot of 2" merely because front and back look different. Research and price ONE object.
...
PHYSICAL ITEM COUNT: {physical_scope}. If ONE_PHYSICAL_ITEM, all photos show ONE physical object from different sides/details. NEVER write lot of 2, set of 2, pair, two pieces, two frames, Number of Items in Set: 2, or equivalent.
...
JSON schema exactly: {{"title":"","price_usd":"","condition_note":"","item_specifics":{{}},"description":"","seo_keywords":[],"seller_check":[]}}"""
    ...
    if physical_scope == "ONE_PHYSICAL_ITEM":
        for _k in list(specifics):
            if any(x in str(_k).casefold() for x in ("number of items", "items in set", "set includes", "number in pack")):
                specifics[_k] = "1"
    ...
    if physical_scope == "ONE_PHYSICAL_ITEM":
        ...
        if re.search(r"\b(?:lot|set|pair)\s+of\s+2\b|\btwo\s+(?:pieces|frames|items|objects)\b|number\s+of\s+items[^0-9]{0,12}2", _count_text):
            return page(label+" draft stop","<h2>Safety stop — AI contradicted the verified physical item count.</h2><p>This lot is classified as ONE physical item, but the draft tried to describe multiple items. Nothing was sent to eBay.</p><p><a href='/picker/review-final-draft'>Return</a></p>"),409
