...
def page(title, body):
    ...
    try:
        active_no = max(3, int(session.get("current_lot_number") or 3))
        active_label = f"LOT {active_no:03d}"
        if active_no >= 3:
            title = str(title).replace("LOT 002", active_label)
            body = str(body).replace("LOT 002", active_label)
    except Exception:
        pass
    ...
    if error:
        ...
        if error.startswith("PICKER_PENDING"):
            return page(
                ...
                """
                <h2>LOT 002 — Google Photos is finishing the selection</h2>
                <p>This is not an error and the selection was not deleted.</p>
                <p>Finish the selection in Google Photos and tap <b>Done</b>. Then press the button below.</p>
                <p><a href="/picker/items"><button style="font-size:20px;padding:12px 18px;">Continue with selected photos</button></a></p>
                """
            ), 202
        ...
        return page(
            "Google Photos error",
            "<h2>Could not load selected photos</h2><pre>" + html.escape(error) + "</pre>"
...
def shipping():
    ...
    body=f'''<h2>LOT 002 — Weight & Shipping</h2><p>✓ Photos: <b>{count}</b> / 24<br>✓ Item weight: <b>{item_weight:.2f} kg</b><br>✓ Estimated packed weight: <b>{packed:.2f} kg</b><br>✓ Item dimensions: <b>{L:g} × {W:g} × {H:g} cm</b><br>✓ Estimated package: <b>{pL:g} × {pW:g} × {pH:g} cm</b><br>✓ Ukrposhta USA parcel tariff estimate: <b>${postage:.2f}</b><br>✓ Recommended eBay shipping charge incl. reserve: <b>${ebay_ship:.2f}</b></p><p style="font-size:14px"><b>Important:</b> USA customs/duty is separate and depends on HS code, declared value and country of origin. It is not invented automatically. Publication stays blocked until that component is verified.</p><p><b>LOT 001 is protected and its shipping policy is not reused.</b></p><form action="/picker/final-listing" method="post">{hidden_field("analysis",analysis)}{hidden_field("identification",identification)}{hidden_field("market",market)}{hidden_field("item_weight",str(item_weight))}{hidden_field("estimated_packed_weight",str(packed))}{hidden_field("dimensions",f"{L:g} x {W:g} x {H:g} cm")}{hidden_field("package_dimensions",f"{pL:g} x {pW:g} x {pH:g} cm")}{hidden_field("shipping_usd",f"{ebay_ship:.2f}")}<p><b>Confirmed material(s):</b><br><input name="confirmed_material" value="{html.escape(confirmed_material)}" required placeholder="e.g. Plastic or Plastic, Metal, String" style="font-size:18px;padding:10px;width:min(520px,90%)"><br><small>Enter only material(s) you can confirm. Multiple materials: separate with commas.</small></p><button type="submit" style="font-size:20px;padding:14px 20px">Create Final eBay Listing Draft</button></form>'''
    return page("LOT 002 Weight & Shipping",body)

@app.route("/picker/final-listing", methods=["POST"])
def final_listing():
    analysis=request.form.get("analysis",""); identification=request.form.get("identification",""); market=request.form.get("market",""); item_weight=request.form.get("item_weight",""); packed_weight=request.form.get("estimated_packed_weight",""); dimensions=request.form.get("dimensions",""); package_dimensions=request.form.get("package_dimensions",""); shipping_usd=request.form.get("shipping_usd",""); confirmed_material=request.form.get("confirmed_material","").strip()
...
Do not invent model, origin, age, condition, completeness or specifications. Preserve all defects and uncertainty from the visual analysis even if the market text is shorter. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    listing_response=call_openai({"model":"gpt-5.6-terra","input":listing_prompt,"reasoning":{"effort":"low"},"max_output_tokens":5000,"store":False},timeout=270)
    ...
    LOT002_DRAFT_CACHE[session.get("picker_session_id")]={
        ...
        "title":(title_match.group(1).strip() if title_match else "1930s USSR Krasnaya Zarya UTA Bakelite Rotary Desk Telephone Untested")[:80],
        "price":price_match.group(1) if price_match else "129.99",
...
def _current_lot_number():
    try:
        return max(3, int(session.get('current_lot_number') or 3))
    except Exception:
        return 3
