...
Do not invent model, origin, age, condition, completeness or specifications. Preserve all defects and uncertainty from the visual analysis even if the market text is shorter. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    ...
    if not title_match:
        return page("Final draft parse stop", f"<h2>Safety stop — title could not be parsed.</h2><p>Nothing was sent to eBay.</p><pre style='white-space:pre-wrap'>{html.escape(listing_response)}</pre>"), 409
    price_note = (
        "<p style='color:#8a4b00'><b>Price was not provided by market research.</b> Enter and confirm the final Buy It Now price below. Nothing is sent to eBay until you confirm it.</p>"
        if not price_match else ""
    )
    return page("LOT 002 Final eBay Listing",f'''<h2>LOT 002 — Final eBay Listing Draft</h2><p style="color:#176b2c"><b>Draft ready.</b> LOT 001 remains protected.</p>{price_note}<pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre><hr><h3>Seller confirmation before eBay Preflight</h3><form action="/picker/confirm-final-draft" method="post"><p><b>Material(s):</b> {html.escape(confirmed_material)}</p><p><label><b>Final Buy It Now price, USD:</b><br><input name="confirmed_price" type="number" min="0.01" step="0.01" value="{html.escape(draft_price)}" required placeholder="Enter price" style="font-size:20px;padding:10px;width:180px"></label></p><label><input type="checkbox" name="confirm_material" value="yes" required> I confirm the material(s) above are correct.</label><br><br><label><input type="checkbox" name="confirm_price" value="yes" required> I confirm this is the price that must be sent to eBay.</label><br><br><button type="submit" style="font-size:18px;padding:14px 18px;font-weight:bold">Confirm → eBay Preflight</button></form>''')
