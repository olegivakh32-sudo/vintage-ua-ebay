...
PHOTO_CACHE = {}
PHOTO_EDIT_CACHE = {}
# Exact seller-approved 2400px JPEG snapshots. These bytes, not a later re-edit, go to eBay.
...
def _safe_photo_edit(image_bytes, fast_clean=True):
    """FAST CLEAN: preserve real item pixels and a light studio background.

    The production default deliberately avoids object cut-out/segmentation.
    This prevents grey/white patches around pale details and makes batches of
    1-24 photos substantially faster. The editor creates one canonical 1600px eBay JPEG per photo and reuses that exact file on approval, so approval does not edit the whole batch a second time. Defects and ruler references stay real.
    """
    ...
    # Only the optional legacy mode performs segmentation. FAST CLEAN is safer
    # for the seller's already-light photo setup and is the default.
    if not fast_clean:
...
def picker_editor():
    ...
    return page("Current Lot Photo Editor", f"""
      ...
      <p>FAST CLEAN: the real item and the existing light studio background are preserved. Only gentle light/contrast/colour/sharpness correction is applied; aggressive background cut-out is disabled. Scratches, cracks, rust, wear and other defects are NOT removed.</p>
      {status}
      {''.join(cards)}
...
def fast_research():
    ...
    if scope == "ONE_PHYSICAL_ITEM" and scope_conf >= 70:
        ...
        ITEM_STRUCTURE_CACHE[picker_id] = structure
    if scope != "ONE_PHYSICAL_ITEM" or scope_conf < 70:
