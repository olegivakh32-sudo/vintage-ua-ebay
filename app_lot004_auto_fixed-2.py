...
def final_listing():
    ...
    label=_current_lot_label() if '_current_lot_label' in globals() else 'LOT 004'
    def market_price(text):
        for pat in [r'(?is)Normal\s+Buy\s+It\s+Now\s*:?\s*\$\s*([0-9]+(?:[.,][0-9]{1,2})?)',r'(?is)Recommended(?:\s+Buy\s+It\s+Now)?\s*(?:price)?\s*:?\s*\$\s*([0-9]+(?:[.,][0-9]{1,2})?)']:
            m=re.search(pat,text or '')
            if m: return m.group(1).replace(',','.')
        return ''
    known_price=market_price(market)
    listing_prompt=f'''Create a complete final eBay.com listing draft in English for {label}.
INITIAL VISUAL ANALYSIS:\n{analysis}\n\nVERIFIED IDENTIFICATION:\n{identification}\n\nMARKET RESEARCH:\n{market}
Item weight: {item_weight} kg; packed weight: {packed_weight} kg; item dimensions: {dimensions}; estimated package: {package_dimensions}; recommended eBay shipping charge: ${shipping_usd} USD from Ukraine to USA.
Known Normal Buy It Now price: ${known_price if known_price else 'use the Normal Buy It Now value in MARKET RESEARCH'}.
Return plain text only with each heading on its own line exactly:
1 TITLE
<one title, max 80 characters>
2 PRICE USD
<digits only, e.g. 49.99>
3 CONDITION
<condition and detailed note>
4 ITEM SPECIFICS
<supported facts; omit unknown optional specifics>
5 DESCRIPTION
<250-450 word buyer-facing description>
6 SEO KEYWORDS
<15-30 relevant phrases>
7 SELLER CHECK BEFORE PUBLISHING
<only genuinely unresolved physical facts; never request data already established above>
Do not invent facts. Preserve defects and uncertainty. Never reuse facts from another lot.'''
    ...
    def section(n,next_n=None):
        txt=listing_response or ''
        m=re.search(rf'(?im)^\s*(?:[#>*_`-]+\s*)*{n}\s*[.)]?\s*(?:[*_`-]+\s*)?(?:TITLE|PRICE\s*USD|CONDITION|ITEM\s*SPECIFICS|DESCRIPTION|SEO\s*KEYWORDS|SELLER\s*CHECK\s*BEFORE\s*PUBLISHING)?[^\n]*\n',txt)
        if not m: return ''
        tail=txt[m.end():]
        if next_n is not None:
            nm=re.search(rf'(?im)^\s*(?:[#>*_`-]+\s*)*{next_n}\s*[.)]?\s+',tail)
            if nm: tail=tail[:nm.start()]
        return tail.strip().strip('*').strip()
    s1=section(1,2)
    title=s1.splitlines()[0].strip(' *#`-–—') if s1 else ''
    if not title:
        m=re.search(r'(?im)^\s*(?:#+\s*)?(?:\*\*)?1\s*[.)]?\s*TITLE(?:\s*[-–—:]\s*|\*\*\s*)(.+?)\s*$',listing_response or '')
        if m: title=m.group(1).strip(' *#`-–—')
    if not title:
        return page(label+' draft stop',f'<h2>Safety stop — title could not be parsed.</h2><p>Nothing was sent to eBay.</p><pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre>'),400
    title=title[:80]
    price=known_price
    if not price:
        m=re.search(r'\$?\s*([0-9]+(?:[.,][0-9]{1,2})?)',section(2,3))
        if m: price=m.group(1).replace(',','.')
    if not price:
        return page(label+' draft stop',f'<h2>Safety stop — market price could not be determined.</h2><p>Nothing was sent to eBay.</p><pre style="white-space:pre-wrap">{html.escape(market)}</pre>'),400
    condition_block=section(3,4)
    description=section(5,6) or listing_response
    LOT002_DRAFT_CACHE[session.get("picker_session_id")]={"listing_text":listing_response,"title":title,"price":price,"condition_note":condition_block[:1000],"description":description,"shipping_usd":shipping_usd,"item_weight":item_weight,"packed_weight":packed_weight,"dimensions":dimensions,"package_dimensions":package_dimensions}
    return page(label+" Final eBay Listing",f'''<h2>{label} — Final eBay Listing Draft</h2><p style="color:#176b2c"><b>Draft ready automatically.</b></p><p><b>Title:</b> {html.escape(title)}<br><b>Buy It Now:</b> ${html.escape(price)}<br><b>Shipping:</b> ${html.escape(shipping_usd)}</p><pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre><hr><p><a href="/ebay/current-preflight" style="display:inline-block;padding:14px 18px;border:1px solid #333;text-decoration:none;font-size:18px;font-weight:bold">Continue to eBay Preflight →</a></p>''')
...
def ebay_current_preflight():
    ...
    try:
        ...
        all_ok=(1<=len(items)<=24 and bool(session.get('photo_editor_approved')) and 1<=main<=len(items) and len(order)==len(items) and bool(order) and order[0]==main and not offers)
        ...
        return page(label+' preflight',f'<h2>{label} — eBay Preflight</h2><p><b>{"READY" if all_ok else "SAFETY STOP"}</b></p><p>SKU: <b>{sku}</b><br>Title: <b>{html.escape(title)}</b><br>Price: <b>${html.escape(price)}</b><br>Category: <b>{html.escape(cat_id)} — {html.escape(cat_name)}</b><br>Condition: <b>{html.escape(cond_enum)} / {html.escape(cond_id)}</b><br>Photos: <b>{len(items)} selected (maximum 24)</b><br>Main photo: <b>source photo {main}</b><br>Photo order: <b>{html.escape(order_text)}</b><br>Shipping: <b>${html.escape(ship)}</b></p><p>Photo order is normalized by capture time / filename, not Google Picker return order.</p>{button}')
    except Exception as exc:
