...
def _safe_photo_edit(image_bytes):
    """Prepare a marketplace photo without redrawing or hiding defects.

    1) whiten only border-connected background; 2) find meaningful dark/non-white
    subject components while rejecting thin edge/table artifacts; 3) place the
    result consistently on a square white canvas. Originals stay untouched.
    """
    ...
    # Gentle global correction only.
    mean_luma = ImageStat.Stat(image.convert("L")).mean[0]
    brightness = 1.10 if mean_luma < 100 else 1.06 if mean_luma < 125 else 0.99 if mean_luma > 215 else 1.01
    image = ImageEnhance.Brightness(image).enhance(brightness)
    image = ImageEnhance.Contrast(image).enhance(1.04)
    image = ImageEnhance.Color(image).enhance(1.01)
    image = ImageEnhance.Sharpness(image).enhance(1.06)

    # Work on a small mask so connected-component analysis is fast on Render.
    w, h = image.size
    scale = min(1.0, 700.0 / max(w, h))
    sw, sh = max(1, int(w * scale)), max(1, int(h * scale))
    small = image.resize((sw, sh), Image.Resampling.LANCZOS)
    pix = small.load()
    fg = bytearray(sw * sh)
    # Anything sufficiently different from white can be part of the object.
    for y in range(sh):
        row = y * sw
        for x in range(sw):
            r, g, b = pix[x, y]
    ...
    for y in range(sh):
        for x in range(sw):
            idx = y * sw + x
            if not fg[idx] or seen[idx]:
                continue
            q = _dq([(x, y)]); seen[idx] = 1
            n = 0; x0 = x1 = x; y0 = y1 = y
            while q:
                cx, cy = q.popleft(); n += 1
                ...
                for nx, ny in ((cx-1,cy),(cx+1,cy),(cx,cy-1),(cx,cy+1)):
                    if 0 <= nx < sw and 0 <= ny < sh:
                        ni=ny*sw+nx
                        if fg[ni] and not seen[ni]:
                            seen[ni]=1; q.append((nx,ny))
            if n >= max(8, int(sw*sh*0.00008)):
                comps.append((n, x0, y0, x1+1, y1+1))

    bbox = None
    if comps:
        comps.sort(reverse=True)
        main = comps[0]
        _, mx0,my0,mx1,my1 = main
        main_area = main[0]
        keep=[]
        for c in comps:
            n,x0,y0,x1,y1=c; cw=x1-x0; ch=y1-y0
            touches = x0==0 or y0==0 or x1==sw or y1==sh
            thin_edge = touches and (cw > 7*max(1,ch) or ch > 7*max(1,cw))
            if thin_edge: continue
            # Keep the main object plus nearby meaningful components (cords etc.).
            gapx=max(0, mx0-x1, x0-mx1); gapy=max(0, my0-y1, y0-my1)
            nearby = gapx <= sw*0.18 and gapy <= sh*0.18
            if c is main or (nearby and n >= max(10, main_area*0.0015)):
                keep.append(c)
        if keep:
            x0=min(c[1] for c in keep); y0=min(c[2] for c in keep)
            x1=max(c[3] for c in keep); y1=max(c[4] for c in keep)
            # Convert mask coordinates back and add a conservative margin.
            x0=int(x0/scale); y0=int(y0/scale); x1=int(x1/scale); y1=int(y1/scale)
            bw=max(1,x1-x0); bh=max(1,y1-y0); margin=int(max(bw,bh)*0.10)
            bbox=(max(0,x0-margin), max(0,y0-margin), min(w,x1+margin), min(h,y1+margin))

    if bbox and (bbox[2]-bbox[0]) > w*0.12 and (bbox[3]-bbox[1]) > h*0.12:
        image=image.crop(bbox)

    # Consistent eBay-ready square: subject fills about 84%, never tiny.
    canvas_size=1600
    max_subject=int(canvas_size*0.84)
    image.thumbnail((max_subject,max_subject), Image.Resampling.LANCZOS)
    canvas=Image.new("RGB", (canvas_size,canvas_size), "white")
    ox=(canvas_size-image.width)//2; oy=(canvas_size-image.height)//2
...
def picker_start():
    ...
    # A genuinely new Google Picker selection starts a new photo-edit state.
...
def picker_items():
    ...
    # Preserve approval when the user merely returns to this page. Reset only
    # when the actual Google Photos selection changed.
    sig_parts = []
    for it in items:
        mf = it.get("mediaFile", {})
        sig_parts.append(str(it.get("id") or mf.get("filename") or mf.get("baseUrl") or ""))
    import hashlib
    ...
    if previous_signature and previous_signature != selection_signature:
        session["photo_editor_approved"] = False
        for key in list(PHOTO_EDIT_CACHE):
            if key[0] == picker_id:
                PHOTO_EDIT_CACHE.pop(key, None)
    session["photo_selection_signature"] = selection_signature
...
Do not invent model, origin, age, condition, completeness or specifications. Preserve all defects and uncertainty from the visual analysis even if the market text is shorter. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    ...
    return page("LOT 002 Final eBay Listing",f'''<h2>LOT 002 — Final eBay Listing Draft</h2><p style="color:#8a5a00"><b>SAFE MODE:</b> LOT 001 is protected. This page does not publish or overwrite any eBay item.</p><pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre>''')
