...
def page(title, body):
    ...
    try:
        ...
        if active_no >= 4:
            title = str(title).replace("LOT 002", active_label).replace("LOT 003", active_label)
            body = str(body).replace("LOT 002", active_label).replace("LOT 003", active_label)
    except Exception:
        pass
...
def picker_analyze():
    ...
    if not (1 <= len(items) <= 24):
        return page(
            ...
            <p>
                Select from 1 to 24 photos again.
            </p>
    ...
    if not session.get("photo_editor_approved"):
        return page(
            "Photo preparation required",
            "<h2>White-background photo preparation is required first.</h2><p><a href='/picker/editor'>Open Photo Editor</a></p>"
        ), 409
...
LOT 002 — ПЕРЕВІРЕНА ІДЕНТИФІКАЦІЯ
...
{html.escape(str(exc))}
    ...
    <p style="color:green;">
        <b>
            ✓ {len(items)} photo(s) + Web Search analyzed
        </b>
    </p>
...
def estimate_item_size_weight_from_photos():
    ...
    items = PHOTO_CACHE.get(picker_id, [])
    if not items:
        return {"weight_kg": "", "length_cm": "", "width_cm": "", "height_cm": "", "confidence": 0}
    content = [{"type":"input_text","text":(
        "Estimate this vintage item's physical size and weight from ALL photos. "
        "This is only a prefill for the seller, not a measured fact. Infer scale from familiar parts and proportions. "
        "Return ONLY compact JSON with numeric fields: weight_kg, length_cm, width_cm, height_cm, confidence. "
        "Use conservative realistic values; confidence is 0-100. No markdown."
    )}]
    for item in items:
        try:
            content.append({"type":"input_image","image_url":google_photo_to_data_url(item,768),"detail":"low"})
        except Exception:
            pass
    try:
        raw=call_openai({"model":"gpt-5.6-luna","input":[{"role":"user","content":content}],"reasoning":{"effort":"low"},"max_output_tokens":300,"store":False},timeout=270)
        m=re.search(r'\{.*\}', raw, re.S)
        data=json.loads(m.group(0) if m else raw)
        out={}
        for k in ("weight_kg","length_cm","width_cm","height_cm"):
            v=float(data.get(k,0)); out[k]=round(v,2) if v>0 else ""
        out["confidence"]=int(float(data.get("confidence",0)))
        return out
    except Exception:
        return {"weight_kg": "", "length_cm": "", "width_cm": "", "height_cm": "", "confidence": 0}
...
- working/non-working
- completeness
- important differences from LOT 002
...
4. НАЙБЛИЖЧИЙ АНАЛОГ ДО LOT 002
...
{html.escape(str(exc))}
    ...
    est_weight = visual_estimate.get("weight_kg", "")
    est_l = visual_estimate.get("length_cm", "")
    est_w = visual_estimate.get("width_cm", "")
    est_h = visual_estimate.get("height_cm", "")
    est_conf = visual_estimate.get("confidence", 0)
    ...
    <p>
        AI has pre-estimated weight and dimensions from the photos.
        <b>Please verify/correct them before shipping.</b>
        Visual estimate confidence: <b>{est_conf}%</b>.
    </p>
    ...
    >
        ...
        <label><b>Estimated length, cm:</b></label><br><br>
        <input type="number" name="dim_l" step="0.1" min="0.1" value="{est_l}" required style="font-size:20px;padding:12px;width:180px"><br><br>
        <label><b>Estimated width, cm:</b></label><br><br>
        <input type="number" name="dim_w" step="0.1" min="0.1" value="{est_w}" required style="font-size:20px;padding:12px;width:180px"><br><br>
        <label><b>Estimated height, cm:</b></label><br><br>
        <input type="number" name="dim_h" step="0.1" min="0.1" value="{est_h}" required style="font-size:20px;padding:12px;width:180px"><br><br>
...
def shipping():
    ...
    confirmed_material=request.form.get("confirmed_material","").strip()
    ...
    if not all([dl,dw,dh]):
        return page("LOT 002 Shipping",f'''<h2>LOT 002 — Weight & Shipping</h2><p>✓ Item weight: <b>{item_weight:.2f} kg</b><br>Estimated packed weight: <b>{packed:.2f} kg</b></p><p><b>Verify/correct the dimensions.</b> The USA postage is calculated automatically. LOT 001 is not reused.</p><form action="/picker/shipping" method="post">{hidden_field("analysis",analysis)}{hidden_field("identification",identification)}{hidden_field("market",market)}{hidden_field("item_weight",str(item_weight))}<p>Length, cm<br><input name="dim_l" value="25.0" type="number" step="0.1" min="0.1" required></p><p>Width, cm<br><input name="dim_w" value="20.0" type="number" step="0.1" min="0.1" required></p><p>Height, cm<br><input name="dim_h" value="20.0" type="number" step="0.1" min="0.1" required></p><button type="submit" style="font-size:18px;padding:12px 16px">Calculate LOT 002 shipping automatically</button></form>''')
    try:
        L,W,H=[float(x.replace(",",".")) for x in (dl,dw,dh)]
        if min(L,W,H)<=0: raise ValueError
    except ValueError: return page("Invalid dimensions","<h2>Use positive dimensions.</h2>"),400
    ...
    count=len(PHOTO_CACHE.get(session.get("picker_session_id"),[]))
    body=f'''<h2>LOT 002 — Weight & Shipping</h2><p>✓ Photos: <b>{count}</b> / 24<br>✓ Item weight: <b>{item_weight:.2f} kg</b><br>✓ Estimated packed weight: <b>{packed:.2f} kg</b><br>✓ Item dimensions: <b>{L:g} × {W:g} × {H:g} cm</b><br>✓ Estimated package: <b>{pL:g} × {pW:g} × {pH:g} cm</b><br>✓ Ukrposhta USA parcel tariff estimate: <b>${postage:.2f}</b><br>✓ Recommended eBay shipping charge incl. reserve: <b>${ebay_ship:.2f}</b></p><p style="font-size:14px"><b>Important:</b> USA customs/duty is separate and depends on HS code, declared value and country of origin. It is not invented automatically. Publication stays blocked until that component is verified.</p><p><b>LOT 001 is protected and its shipping policy is not reused.</b></p><form action="/picker/final-listing" method="post">{hidden_field("analysis",analysis)}{hidden_field("identification",identification)}{hidden_field("market",market)}{hidden_field("item_weight",str(item_weight))}{hidden_field("estimated_packed_weight",str(packed))}{hidden_field("dimensions",f"{L:g} x {W:g} x {H:g} cm")}{hidden_field("package_dimensions",f"{pL:g} x {pW:g} x {pH:g} cm")}{hidden_field("shipping_usd",f"{ebay_ship:.2f}")}<fieldset style="padding:12px;margin:14px 0"><legend><b>SELLER CONFIRMED FACTS</b></legend><p><b>Material(s) — select one or several:</b></p><label><input type="checkbox" name="material_choice" value="Plastic"> Plastic</label> &nbsp; <label><input type="checkbox" name="material_choice" value="Metal"> Metal</label> &nbsp; <label><input type="checkbox" name="material_choice" value="Wood"> Wood</label><br><label><input type="checkbox" name="material_choice" value="Glass"> Glass</label> &nbsp; <label><input type="checkbox" name="material_choice" value="Ceramic"> Ceramic</label> &nbsp; <label><input type="checkbox" name="material_choice" value="Fabric/Textile"> Fabric/Textile</label><br><label><input type="checkbox" name="material_choice" value="Paper/Cardboard"> Paper/Cardboard</label> &nbsp; <label><input type="checkbox" name="material_choice" value="Rubber"> Rubber</label> &nbsp; <label><input type="checkbox" name="material_choice" value="Leather"> Leather</label> &nbsp; <label><input type="checkbox" name="material_choice" value="String/Cord"> String/Cord</label><p>Other material (optional):<br><input name="material_other" placeholder="e.g. Bakelite" style="font-size:17px;padding:8px;width:min(420px,90%)"></p><p><small>One choice = specific material. Several choices = combined materials. These facts override AI/web guesses.</small></p><p><b>Working / tested status:</b><br><select name="seller_tested_status" required style="font-size:17px;padding:8px"><option value="Untested">Untested</option><option value="Tested working">Tested working</option><option value="Tested not working">Tested not working</option><option value="Not applicable">Not applicable</option></select></p><p><b>Completeness:</b><br><select name="seller_completeness" style="font-size:17px;padding:8px"><option value="Unknown">Unknown</option><option value="Complete">Complete</option><option value="Incomplete">Incomplete</option></select></p><p><b>Brand / maker if physically confirmed:</b><br><input name="seller_brand" placeholder="leave blank if not confirmed" style="font-size:17px;padding:8px;width:min(420px,90%)"></p><p><b>Country of origin if physically confirmed:</b><br><input name="seller_origin" placeholder="leave blank if not confirmed" style="font-size:17px;padding:8px;width:min(420px,90%)"></p></fieldset><button type="submit" style="font-size:20px;padding:14px 20px">Create Final eBay Listing Draft</button></form>'''
    return page("LOT 002 Weight & Shipping",body)
...
def final_listing():
    ...
    materials=[x.strip() for x in request.form.getlist("material_choice") if x.strip()]
    other=request.form.get("material_other","").strip()
    if other: materials.append(other)
    materials=list(dict.fromkeys(materials))
    confirmed_material=", ".join(materials)
    seller_tested_status=request.form.get("seller_tested_status","Untested").strip() or "Untested"
    seller_completeness=request.form.get("seller_completeness","Unknown").strip() or "Unknown"
    seller_brand=request.form.get("seller_brand","").strip()
    seller_origin=request.form.get("seller_origin","").strip()
    if not confirmed_material:
        return page("Material required","<h2>Confirm the item material before creating the final draft.</h2><p>Nothing was sent to eBay.</p>"),400
    listing_prompt=f'''Create a complete final eBay.com listing draft in English for LOT 002.
INITIAL VISUAL ANALYSIS:\n{analysis}\n\nVERIFIED IDENTIFICATION:\n{identification}\n\nMARKET RESEARCH:\n{market}
Item weight: {item_weight} kg; packed weight: {packed_weight} kg; item dimensions: {dimensions}; estimated package: {package_dimensions}; recommended eBay shipping charge: ${shipping_usd} USD from Ukraine to USA. USA customs/duty remains unverified and must not be invented.
SELLER-CONFIRMED MATERIAL(S): {confirmed_material}. This is authoritative. Use exactly these confirmed material(s) in title/Item Specifics/description where relevant. Do NOT call the item wooden unless Wood is explicitly included here. Do not invent additional materials.
SELLER-CONFIRMED TESTED STATUS: {seller_tested_status}. This overrides AI/web assumptions.
SELLER-CONFIRMED COMPLETENESS: {seller_completeness}. This overrides AI/web assumptions when not Unknown.
SELLER-CONFIRMED BRAND/MAKER: {seller_brand or 'Not physically confirmed'}. Do not upgrade a web hypothesis to a fact.
SELLER-CONFIRMED COUNTRY OF ORIGIN: {seller_origin or 'Not physically confirmed'}. Do not invent an origin.

Return plain text only with these sections:
1 TITLE — max 80 characters, strong eBay SEO, no unsupported claims.
2 PRICE USD — use the Normal/recommended Buy It Now price from market research exactly.
3 CONDITION — eBay condition plus a detailed seller condition note.
4 ITEM SPECIFICS — as complete as evidence allows.
5 DESCRIPTION — IMPORTANT: write a useful detailed 250-450 word buyer-facing description, not a short summary. Include identification/manufacturer/model only to the verified confidence level; historical origin/era; materials/design; what is included; condition; every visible defect and uncertainty; tested/untested status; restoration/parts/display suitability; dimensions and weight; shipping from Ukraine; and a clear request to review all photos. Do not repeat unsupported market claims or sold comps in the buyer description.
6 SEO KEYWORDS — 15-30 relevant phrases, no keyword spam or false terms.
7 SELLER CHECK BEFORE PUBLISHING — concise list of any remaining uncertainty or manual verification needed.

Do not invent model, origin, age, condition, completeness or specifications. Preserve all defects and uncertainty from the visual analysis even if the market text is shorter. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    listing_response=call_openai({"model":"gpt-5.6-terra","input":listing_prompt,"reasoning":{"effort":"low"},"max_output_tokens":3400,"store":False},timeout=270)
    # Keep the final LOT002 draft server-side; never put the long draft into the signed Flask cookie.
    title_match=re.search(r'(?mi)^\s*1\s+TITLE\s*\n([^\n]+)',listing_response)
    price_match=re.search(r'(?mi)^\s*2\s+PRICE USD\s*\n\s*\$?([0-9]+(?:\.[0-9]{1,2})?)',listing_response)
    condition_block=_lot002_extract_section(listing_response,3,4) if '_lot002_extract_section' in globals() else ''
    description=_lot002_extract_section(listing_response,5,6) if '_lot002_extract_section' in globals() else listing_response
    LOT002_DRAFT_CACHE[session.get("picker_session_id")]={
        "listing_text":listing_response,
        "title":(title_match.group(1).strip() if title_match else "")[:80],
        "price":price_match.group(1) if price_match else "",
        "condition_note":condition_block[:1000],
        "description":description,
        "shipping_usd":shipping_usd,
        "confirmed_material":confirmed_material,
        "seller_tested_status":seller_tested_status,
        "seller_completeness":seller_completeness,
        "seller_brand":seller_brand,
        "seller_origin":seller_origin,
        "item_weight":item_weight,"packed_weight":packed_weight,"dimensions":dimensions,"package_dimensions":package_dimensions,
    }
    draft_price=(price_match.group(1) if price_match else "")
    if not title_match or not price_match:
        return page("Final draft parse stop", f"<h2>Safety stop — title or price could not be parsed.</h2><p>Nothing was sent to eBay.</p><pre style='white-space:pre-wrap'>{html.escape(listing_response)}</pre>"), 409
    return page("LOT 002 Final eBay Listing",f'''<h2>LOT 002 — Final eBay Listing Draft</h2><p style="color:#176b2c"><b>Draft ready.</b> LOT 001 remains protected.</p><pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre><hr><h3>Seller confirmation before eBay Preflight</h3><form action="/picker/confirm-final-draft" method="post"><p><b>Material(s):</b> {html.escape(confirmed_material)}</p><p><label><b>Final Buy It Now price, USD:</b><br><input name="confirmed_price" type="number" min="0.01" step="0.01" value="{html.escape(draft_price)}" required style="font-size:20px;padding:10px;width:180px"></label></p><label><input type="checkbox" name="confirm_material" value="yes" required> I confirm the material(s) above are correct.</label><br><br><label><input type="checkbox" name="confirm_price" value="yes" required> I confirm this is the price that must be sent to eBay.</label><br><br><button type="submit" style="font-size:18px;padding:14px 18px;font-weight:bold">Confirm → eBay Preflight</button></form>''')

@app.route("/picker/confirm-final-draft", methods=["POST"])
def confirm_final_draft():
    draft=LOT002_DRAFT_CACHE.get(session.get("picker_session_id")) or {}
    if not draft:
        return page("Draft missing","<h2>Draft is missing. Nothing was sent to eBay.</h2>"),400
    if request.form.get("confirm_material")!="yes" or request.form.get("confirm_price")!="yes":
        return page("Confirmation required","<h2>Material and price must both be confirmed.</h2><p>Nothing was sent to eBay.</p>"),400
    raw=request.form.get("confirmed_price","").strip().replace(",",".")
    try:
        val=float(raw)
        if val<=0 or val>999999: raise ValueError
    except ValueError:
        return page("Invalid price","<h2>Enter a valid positive USD price.</h2><p>Nothing was sent to eBay.</p>"),400
    price=f"{val:.2f}"
    draft["price"]=price
    draft["price_confirmed"]=True
    draft["material_confirmed"]=True
...
LOT_COUNTER_SCHEMA_VERSION = "stable-v3"
...
def _sync_current_lot_once():
    if session.get('lot_counter_schema') != LOT_COUNTER_SCHEMA_VERSION:
        session['current_lot_number'] = _discover_next_lot_from_ebay()
        session['lot_counter_schema'] = LOT_COUNTER_SCHEMA_VERSION
    return int(session.get('current_lot_number') or 4)
...
def _build_logical_photo_order(items, main_index):
    ...
    rest=[i for i in indexed if i!=main_index]
    rest.sort(key=lambda i:_photo_capture_value(items[i-1],i))
...
def _current_condition(token,category_id,draft):
    ...
    for pol in (r.json() or {}).get('itemConditionPolicies') or []:
        if str(pol.get('categoryId') or '')==str(category_id):
            ...
            break
    text=((draft.get('condition_note') or '')+' '+(draft.get('listing_text') or '')).casefold()
    broken=any(x in text for x in ('not working','non-working','for parts','parts or repair','repair or parts'))
    pref=['7000','6000','5000','4000','3000'] if broken else ['3000','4000','5000','6000','7000']
...
def _current_aspects(draft):
    ...
    for line in block.splitlines():
        ...
        if not k or not v or len(k)>65 or len(v)>65:
            continue
        if any(x in low for x in ('unknown','not verified','uncertain','not applicable','does not apply','n/a')):
            continue
        out[k]=[v]
    material=str(draft.get('confirmed_material') or '').strip()
    if material:
        vals=[x.strip() for x in material.split(',') if x.strip()]
        if vals:
            out['Material']=vals
    return out
...
def ebay_current_preflight():
    ...
    try:
        ...
        all_ok=(1<=len(items)<=24 and bool(session.get('photo_editor_approved')) and 1<=main<=len(items) and len(order)==len(items) and bool(order) and order[0]==main and not offers and bool(draft.get('price_confirmed')) and bool(draft.get('material_confirmed')) and bool(draft.get('confirmed_material')))
...
def ebay_current_setup():
    ...
    if not draft or request.form.get('shipping_verified')!='yes' or not draft.get('price_confirmed') or not draft.get('material_confirmed') or not draft.get('confirmed_material'):
        return page(label+' setup','<h2>Safety stop — draft, shipping, seller-confirmed price, or seller-confirmed material is missing.</h2><p>Nothing was sent to eBay.</p>'),400
    try:
        token=get_ebay_access_token()
...
def ebay_current_publish():
    ...
    try:
        ...
        if len(offers)!=1:
            return page(label+' publish stop',f'<h2>Expected one Offer, found {len(offers)}.</h2>'),409
...
def next_lot():
    old_picker=session.get('picker_session_id'); old_sku=_current_lot_sku()
    try:
        ...
        pass
    session['current_lot_number']=_current_lot_number()+1
    session['lot_counter_schema']=LOT_COUNTER_SCHEMA_VERSION
    for key in ('picker_session_id','picker_uri','photo_editor_approved','main_photo_index','photo_order','photo_selection_signature','shipping_usd','item_weight','item_dimensions'):
