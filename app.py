...
def _safe_photo_edit(image_bytes):
    """Gentle eBay-safe enhancement. Never removes defects or changes object content."""
...
def picker_editor():
    ...
    return page("LOT 002 Photo Editor", f"""
      ...
      <p>Gentle automatic correction only: light, contrast, colour, sharpness and size. Scratches, cracks, rust, wear and other defects are NOT removed.</p>
      {status}
      {''.join(cards)}
...
def picker_items():
    ...
    # Drop stale edited previews for this picker session.
    ...
    if 1 <= count <= 24:
        ...
        button = """
        <p><a href="/picker/editor" style="display:inline-block;padding:12px 16px;border:1px solid #777;border-radius:7px;text-decoration:none">✨ Auto Photo Editor — Preview / Approve</a></p>
        <form
...
def shipping():
    ...
    if not analysis or not identification or not market:
        return page("Missing LOT data", "<h2>LOT data missing</h2><p>Please return to the previous step.</p>"), 400
    try: item_weight = float(item_weight_raw.replace(",", "."))
    except (TypeError, ValueError): return page("Invalid weight", "<h2>Invalid weight</h2><p>Enter kilograms.</p>"), 400
    if item_weight <= 0: return page("Invalid weight", "<h2>Weight must be greater than 0 kg.</h2>"), 400
    if item_weight <= 0.5: packaging_allowance = 0.30
    elif item_weight <= 1.0: packaging_allowance = 0.45
    elif item_weight <= 2.0: packaging_allowance = 0.65
    elif item_weight <= 3.0: packaging_allowance = 0.85
    elif item_weight <= 5.0: packaging_allowance = 1.10
    elif item_weight <= 10.0: packaging_allowance = 1.60
    else: packaging_allowance = max(2.0, item_weight * 0.18)
    estimated_packed_weight = round(item_weight + packaging_allowance, 2)
    dim_l=request.form.get("dim_l","").strip(); dim_w=request.form.get("dim_w","").strip(); dim_h=request.form.get("dim_h","").strip()
    actual_shipping_uah=request.form.get("actual_shipping_uah","").strip()
    if not all([dim_l,dim_w,dim_h,actual_shipping_uah]):
        return page("LOT 002 Shipping", f'''<h2>LOT 002 — Weight & Shipping</h2>
        <p>✓ Item weight: <b>{item_weight:.2f} kg</b><br>Estimated packed weight: <b>{estimated_packed_weight:.2f} kg</b></p>
        <p><b>AI photo estimate is prefilled below. Verify/correct it, then enter the actual Ukrposhta Ukraine → USA price.</b><br>LOT 001 clock data is intentionally not reused.</p>
        <form action="/picker/shipping" method="post">{hidden_field("analysis",analysis)}{hidden_field("identification",identification)}{hidden_field("market",market)}{hidden_field("item_weight",str(item_weight))}
        <p>Length, cm<br><input name="dim_l" value="{html.escape(dim_l)}" type="number" step="0.1" min="0.1" required></p>
        <p>Width, cm<br><input name="dim_w" value="{html.escape(dim_w)}" type="number" step="0.1" min="0.1" required></p>
        <p>Height, cm<br><input name="dim_h" value="{html.escape(dim_h)}" type="number" step="0.1" min="0.1" required></p>
        <p>Actual Ukrposhta cost to USA, UAH<br><input name="actual_shipping_uah" type="number" step="0.01" min="0.01" required></p>
        <button type="submit" style="font-size:18px;padding:12px 16px">Calculate LOT 002 shipping</button></form>''')
    try:
        L,W,H=[float(x.replace(",",".")) for x in (dim_l,dim_w,dim_h)]; ship_uah=float(actual_shipping_uah.replace(",","."))
        if min(L,W,H,ship_uah)<=0: raise ValueError
    except ValueError: return page("Invalid shipping data","<h2>Use positive numbers for dimensions and shipping.</h2>"),400
    pack_L,pack_W,pack_H=round(L+10,1),round(W+10,1),round(H+10,1)
    photo_count=len(PHOTO_CACHE.get(session.get("picker_session_id"),[]))
    body=f'''<h2>LOT 002 — Weight & Shipping</h2><p>✓ Photos: <b>{photo_count}</b> (maximum 24)<br>✓ Item weight: <b>{item_weight:.2f} kg</b><br>✓ Estimated packed weight: <b>{estimated_packed_weight:.2f} kg</b><br>✓ Item dimensions: <b>{L:g} × {W:g} × {H:g} cm</b><br>✓ Estimated package: <b>{pack_L:g} × {pack_W:g} × {pack_H:g} cm</b><br>✓ Actual Ukrposhta cost entered: <b>{ship_uah:.2f} UAH</b></p><p><b>No LOT 001 shipping policy is changed or reused.</b></p>
    <form action="/picker/final-listing" method="post">{hidden_field("analysis",analysis)}{hidden_field("identification",identification)}{hidden_field("market",market)}{hidden_field("item_weight",str(item_weight))}{hidden_field("estimated_packed_weight",str(estimated_packed_weight))}{hidden_field("dimensions",f"{L:g} x {W:g} x {H:g} cm")}{hidden_field("package_dimensions",f"{pack_L:g} x {pack_W:g} x {pack_H:g} cm")}{hidden_field("actual_shipping_uah",f"{ship_uah:.2f}")}<button type="submit" style="font-size:20px;padding:14px 20px">Create Final eBay Listing Draft</button></form>'''
    return page("LOT 002 Weight & Shipping",body)

@app.route("/picker/final-listing", methods=["POST"])
def final_listing():
    analysis=request.form.get("analysis",""); identification=request.form.get("identification",""); market=request.form.get("market",""); item_weight=request.form.get("item_weight",""); packed_weight=request.form.get("estimated_packed_weight",""); dimensions=request.form.get("dimensions",""); package_dimensions=request.form.get("package_dimensions",""); shipping_uah=request.form.get("actual_shipping_uah","")
    listing_prompt=f'''Create a complete final eBay.com listing draft in English for LOT 002.
INITIAL VISUAL ANALYSIS:\n{analysis}\n\nVERIFIED IDENTIFICATION:\n{identification}\n\nMARKET RESEARCH:\n{market}
Item weight: {item_weight} kg; packed weight: {packed_weight} kg; item dimensions: {dimensions}; estimated package: {package_dimensions}; actual shipping cost entered by seller: {shipping_uah} UAH Ukraine to USA.
