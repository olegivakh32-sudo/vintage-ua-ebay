...
{{"scope":"ONE_PHYSICAL_ITEM|MULTIPLE_PHYSICAL_ITEMS|UNCERTAIN","confidence":0,"note":"short Ukrainian explanation"}}"""
    ...
    try:
        ...
        conf = 0
    result={"scope":scope,"confidence":conf,"note":str(obj.get("note") or "").strip()}
...
def fast_research():
    ...
    if scope != "ONE_PHYSICAL_ITEM" or scope_conf < 70:
        msg = html.escape(structure.get("note") or "The selected photos may not represent one sellable lot.")
        return page(label+" — Scope review", f"""<h2>{label} — Safety Stop before market research</h2>
        <p><b>Physical-item check:</b> {html.escape(scope)} ({scope_conf}%)</p>
        <p>{msg}</p>
        <p>Market research was <b>not</b> run, so no incorrect price was generated.</p>
        <p><a href='/picker/items' style='display:inline-block;padding:12px 16px;border:1px solid #333;text-decoration:none'>Review selected photos</a></p>"""), 409
...
The four prices must be numeric USD whenever listing is reasonably possible."""}]
    ...
    <p><b>Suggested listing price ({html.escape(price_label)}):</b> {('$'+html.escape(price)) if price else 'not parsed — you can correct it before Preflight'}</p><p><b>Physical-item check:</b> {html.escape(scope)} ({scope_conf}%)</p>
