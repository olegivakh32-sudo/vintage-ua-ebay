...
def _market_price_tiers(market_text):
    """Return researched price tiers as numeric strings when present."""
    quick = _market_price_value(market_text, [r"Quick\s+sale", r"Швидк(?:ий|а)[^\n\r]{0,20}(?:продаж|ціна)"])
    normal = _market_price_value(market_text, [
        r"Normal\s+Buy\s+It\s+Now", r"Normal\s+BIN", r"Recommended\s+(?:BIN|Buy\s+It\s+Now)",
        r"Нормальн(?:а|ий|е)[^\n\r]{0,30}(?:Buy\s+It\s+Now|BIN|ціна)?",
    ])
    patient = _market_price_value(market_text, [r"Patient\s+seller", r"Терпляч(?:ий|ого)[^\n\r]{0,20}(?:продав|ціна)"])
    minimum = _market_price_value(market_text, [r"Minimum\s+acceptable\s+offer", r"Minimum\s+offer", r"Мінімальн[^\n\r]{0,30}(?:пропоз|ціна)"])
    return {"quick": quick, "normal": normal, "patient": patient, "minimum": minimum}
...
Use MULTIPLE_UNRELATED_ITEMS only when the photos clearly show different unrelated goods that should not be researched as one listing.
Use ONE_SELLABLE_SET when multiple physical pieces clearly belong together as the intended set/kit/lot.
Use UNCERTAIN only when you truly cannot decide.
Return ONLY valid JSON:
{{"scope":"ONE_ITEM_DIFFERENT_VIEWS|ONE_SELLABLE_SET|MULTIPLE_UNRELATED_ITEMS|UNCERTAIN","confidence":0,"note":"short Ukrainian explanation"}}"""
    ...
    if scope not in {"ONE_ITEM_DIFFERENT_VIEWS","ONE_SELLABLE_SET","MULTIPLE_UNRELATED_ITEMS","UNCERTAIN"}:
        scope = "UNCERTAIN"
    try:
        conf = max(0, min(100, int(float(obj.get("confidence") or 0))))
    except Exception:
...
def fast_research():
    ...
    if scope == "MULTIPLE_UNRELATED_ITEMS" or (scope == "UNCERTAIN" and scope_conf < 70):
        ...
        <p><b>Sellable-scope check:</b> {html.escape(scope)} ({scope_conf}%)</p>
        <p>{msg}</p>
...
CRITICAL SCOPE RULE: all selected photos belong to ONE intended sellable lot. Do NOT reinterpret front/back views, backing pieces, attached/removable components, detail views, ruler shots, or alternate angles as separate products. If scope is ONE_ITEM_DIFFERENT_VIEWS, research it as one object unless the photos clearly prove a missing/broken component. If scope is ONE_SELLABLE_SET, research the set as one listing.
...
The four prices must be numeric USD whenever listing is reasonably possible."""}]
    ...
    <p><b>Suggested listing price ({html.escape(price_label)}):</b> {('$'+html.escape(price)) if price else 'not parsed — you can correct it before Preflight'}</p><p><b>Sellable-scope check:</b> {html.escape(scope)} ({scope_conf}%)</p>
