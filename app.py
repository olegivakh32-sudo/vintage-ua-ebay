...
def _white_background_edge_connected(image):
    ...
    # Protect the actual object envelope.  A flood-fill alone can leak into
    # white/cream item details that physically touch the background (white
    # shoes, porcelain edges, paper labels, etc.).  Build a conservative
    # foreground core from pixels clearly different from the border colour,
    # then forbid background replacement inside its expanded bounding box.
    core = Image.new("L", (sw, sh), 0)
    cp = core.load()
    core_tol = max(28.0, tol * 0.72)
    xs, ys = [], []
    for yy in range(sh):
        for xx in range(sw):
            c = px[xx, yy]
            d0, d1, d2 = int(c[0])-med[0], int(c[1])-med[1], int(c[2])-med[2]
            dist = (d0*d0 + d1*d1 + d2*d2) ** 0.5
            if dist > core_tol:
                cp[xx, yy] = 255
                xs.append(xx); ys.append(yy)
    if xs and ys:
        # Protect only pixels close to a confidently detected foreground core,
        # NOT the whole rectangular object envelope.  The old rectangle kept
        # pale background inside/around the object unchanged.  A dilated core
        # preserves white/cream object details (shoes, porcelain edges, labels)
        # when they touch coloured parts, while still allowing the surrounding
        # background to become pure white.
        radius = max(5, int(max(sw, sh) * 0.018))
        # PIL MaxFilter requires an odd kernel size.
        kernel = min(51, radius * 2 + 1)
        if kernel % 2 == 0:
            kernel += 1
        protected = core.filter(ImageFilter.MaxFilter(kernel))
        pp = protected.load()
        for yy in range(sh):
            for xx in range(sw):
                if pp[xx, yy] > 0:
...
def _safe_photo_edit(image_bytes, fast_clean=False):
    """Seller-safe eBay editor.

    Default mode performs conservative border-connected background cleanup to
    true white, then light tonal/sharpness correction. It never globally
    replaces light colours, so pale item details and ruler/defect evidence are
    preserved. The canonical 1600px JPEG shown in Edited preview is cached and
    the exact same bytes are used after approval and for eBay upload.
    """
    ...
    # Default production mode performs conservative border-connected cleanup.
    # fast_clean=True remains available as a fallback for unusually difficult photos.
    if not fast_clean:
        image = _white_background_edge_connected(image)

    mean_luma = ImageStat.Stat(image.convert("L")).mean[0]
    if mean_luma < 95: brightness = 1.10
    elif mean_luma < 120: brightness = 1.06
    elif mean_luma > 215: brightness = 0.99
    else: brightness = 1.015
    ...
    # 2000 px is ample for eBay listing photos and materially reduces CPU,
    # memory and upload time compared with repeatedly processing 2400 px files.
    if max(image.size) > 1600:
        image.thumbnail((1600, 1600), Image.Resampling.LANCZOS)
    out = BytesIO()
    image.save(out, format="JPEG", quality=92, optimize=False)
    return out.getvalue()
...
def picker_editor():
    ...
    for i in range(1, len(items) + 1):
        ...
        <div style='border:1px solid #ddd;border-radius:10px;padding:8px;margin-bottom:14px'>
          ...
          <div style='display:grid;grid-template-columns:1fr 1fr;gap:8px'>
            ...
            <div><b>Edited</b><br><img src='/picker/edited/{i}?v=5' loading='lazy' style='width:100%;max-height:280px;object-fit:contain;background:#f4f4f4'></div>
          </div>
        </div>""")
    ...
    return page("Current Lot Photo Editor", f"""
      ...
      <p>AUTO WHITE: conservative border-connected background cleanup to white + gentle light/contrast/colour/sharpness. The item, ruler references, scratches, cracks, rust, wear and other defects are preserved.</p>
      {status}
      {''.join(cards)}
      <form id='photo-approval-form' method='post' action='/picker/editor/approve'>
        <button type='submit' style='font-size:18px;padding:14px 18px'>Approve photos — MAIN PHOTO auto if not selected</button>
      </form>
