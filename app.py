...
<body>
  <div class="shell">
    <div class="topbar"><div><div class="brand">Vintage UA eBay Machine</div><div class="subtitle">Fast listing workflow • safety checked before publish</div></div></div>
...
def _market_bin_price(market_text):
    """Return the research Normal BIN; derive a conservative value only if needed."""
    normal = _market_price_value(market_text, [
        r"Normal\s+Buy\s+It\s+Now",
        r"Normal\s+BIN",
        r"Recommended\s+(?:BIN|Buy\s+It\s+Now)",
    ...
    try:
        q = float(quick) if quick else None
        pt = float(patient) if patient else None
        if q and pt and pt >= q:
            # Normal market ask: closer to quick-sale than optimistic patient price.
            return f"{(q + 0.58 * (pt-q)):.2f}"
        if q:
            return f"{q*1.18:.2f}"
        if pt:
            return f"{pt*0.82:.2f}"
...
Rules: visual evidence overrides web similarity; never invent maker/model/year/country/material; distinguish maker marks from quality/state marks; clearly separate confirmed SOLD evidence from ACTIVE asking prices; never call an active listing sold; if exact sold comps do not exist, derive a conservative BIN from closest analogs and label confidence.
...
def ebay_current_preflight():
    ...
    try:
        ...
        base_aspects=_current_aspects(draft); resolved_aspects, missing_required=_fill_required_aspects(token,cat_id,draft,base_aspects)
