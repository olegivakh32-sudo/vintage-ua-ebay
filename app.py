...
def picker_items():
    ...
    if error:
        # Google Picker sessions are temporary. Never expose the raw Google JSON
        # or trap the user on an expired/empty session.
        if "FAILED_PRECONDITION" in error or "has not picked media items" in error or "session has expired" in error:
            session.pop("picker_session_id", None)
            return page(
                "Photo selection expired",
                """
                <h2>LOT 002 — photo selection expired</h2>
                <p>Google Photos closed the temporary Picker session. LOT 001 is not affected.</p>
                <p><a href="/picker/start?new=1"><button style="font-size:20px;padding:12px 18px;">Select LOT 002 photos again</button></a></p>
                """
            ), 409
        return page(
            "Google Photos error",
            "<h2>Could not load selected photos</h2><p>Please reopen photo selection.</p>"
            '<p><a href="/picker/start?new=1">Open Google Photos Picker</a></p>'
        ), 502
    ...
    # Preserve editor approval on ordinary refresh/back navigation. Reset only
    # when Google actually returns a different selection for this Picker session.
    previous_items = PHOTO_CACHE.get(picker_id, [])
    def _selection_signature(seq):
        sig=[]
        for obj in seq or []:
            mf=obj.get("mediaFile") or {}
            sig.append((str(obj.get("id") or ""), str(mf.get("filename") or ""), str(mf.get("baseUrl") or "")))
        return tuple(sig)
    selection_changed = bool(previous_items) and _selection_signature(previous_items) != _selection_signature(items)
    ...
    for pol in r.json().get('itemConditionPolicies') or []:
        if str(pol.get('categoryId') or '') == LOT002_CATEGORY_ID:
            for c in pol.get('itemConditions') or []:
                if str(c.get('conditionId') or '') == '3000' or str(c.get('conditionEnum') or '') == 'USED':
                    return True, 'USED / 3000'
    return False, 'USED condition not returned for category'
...
def ebay_publish_lot002():
    ...
    try:
        ...
        checks=[
            ...
            ('Condition',inv.get('condition'),'USED',inv.get('condition')=='USED'),
