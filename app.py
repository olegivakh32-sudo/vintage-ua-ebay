...
def picker_editor():
    ...
    return page("LOT 002 Photo Editor", f"""
      <h2>LOT 002 — Auto Photo Editor</h2>
      ...
      {status}
      {''.join(cards)}
      <form method='post' action='/picker/editor/approve'>
        <button type='submit' style='font-size:18px;padding:14px 18px'>Approve edited {len(items)} photo(s) for eBay</button>
      </form>
...
Do not invent model, origin, age, condition, completeness or specifications. Preserve all defects and uncertainty from the visual analysis even if the market text is shorter. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    ...
    return page("LOT 002 Final eBay Listing",f'''<h2>LOT 002 — Final eBay Listing Draft</h2><p style="color:#176b2c"><b>Draft ready.</b> LOT 001 remains protected.</p><pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre><hr><p><a href="/ebay/lot002-preflight" style="display:inline-block;padding:14px 18px;border:1px solid #333;text-decoration:none;font-size:18px;font-weight:bold">Continue to eBay Preflight →</a></p>''')
...
def _lot002_upload_edited_photos(token):
    ...
    for i,item in enumerate(items,1):
        ...
        results.append({'number':i,'filename':filename,'image_id':image_id,'image_url':image_url})
...
def ebay_lot002_preflight():
    ...
    try:
        ...
        all_ok=(len(items)==24 and bool(session.get('photo_editor_approved')) and used_ok and not offers and draft.get('shipping_usd'))
        status='READY FOR CONTROLLED SETUP' if all_ok else 'SAFETY STOP'
...
def ebay_lot002_setup():
    ...
    try:
        ...
        chk=requests.get(inv_url,headers=_lot002_headers(token),timeout=30)
        ...
        ir=requests.put(inv_url,headers=_lot002_headers(token,True),json=inv_payload,timeout=60)
        ...
        orr=requests.post('https://api.ebay.com/sell/inventory/v1/offer',headers=_lot002_headers(token,True),json=offer_payload,timeout=60)
...
def ebay_lot002_fix_price():
    ...
    try:
        ...
        payload = {'sku':offer.get('sku'),'marketplaceId':offer.get('marketplaceId'),'format':offer.get('format'),'availableQuantity':offer.get('availableQuantity'),'categoryId':offer.get('categoryId'),'merchantLocationKey':offer.get('merchantLocationKey'),'listingDuration':offer.get('listingDuration') or 'GTC','listingPolicies':offer.get('listingPolicies') or {},'pricingSummary':{'price':{'value':target_price,'currency':'USD'}}}
        ...
        if fj.get('status')!='UNPUBLISHED' or fj.get('sku')!=LOT002_SKU or fresh_price!=target_price:
        ...
        return page('LOT 002 price corrected',f"""<h2>LOT 002 — PRICE CORRECTED</h2><p>Verified directly from eBay:<br>Offer ID: <b>{html.escape(offer_id)}</b><br>Price: <b>${html.escape(fresh_price)}</b><br>Status: <b>UNPUBLISHED</b></p><p><b>Nothing is live yet.</b></p><p><a href=\"/ebay/publish-lot002\" style=\"font-size:18px\">Run final eBay verification →</a></p>""")
    except Exception as exc:
