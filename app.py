...
Do not invent model, origin, age, condition, completeness or specifications. Preserve all defects and uncertainty from the visual analysis even if the market text is shorter. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    ...
    return page("LOT 002 Final eBay Listing",f'''<h2>LOT 002 — Final eBay Listing Draft</h2><p style="color:#176b2c"><b>Draft ready.</b> LOT 001 remains protected.</p><pre style="white-space:pre-wrap">{html.escape(listing_response)}</pre><hr><p><a href="/ebay/current-preflight" style="display:inline-block;padding:14px 18px;border:1px solid #333;text-decoration:none;font-size:18px;font-weight:bold">Continue to eBay Preflight →</a></p>''')
...
def ebay_publish_lot002():
    ...
    try:
        ...
        checks=[
            ...
            ('EPS photos',len(images),24,len(images)==24),
...
def _current_upload_photos(token):
    ...
    if len(items)!=24 or not session.get('photo_editor_approved'):
        raise RuntimeError('Exactly 24 approved photos are required.')
    ...
    if len(order)!=24 or sorted(order)!=list(range(1,25)) or order[0]!=main:
...
def ebay_current_preflight():
    ...
    try:
        ...
        draft.update({'category_id':cat_id,'category_name':cat_name,'condition_id':cond_id,'condition_enum':cond_enum}); LOT002_DRAFT_CACHE[session.get('picker_session_id')]=draft
        offers=_current_get_offers(token); main=int(session.get('main_photo_index') or 0); order=session.get('photo_order') or []
        all_ok=(len(items)==24 and bool(session.get('photo_editor_approved')) and 1<=main<=24 and len(order)==24 and order[0]==main and not offers)
        ...
        return page(label+' preflight',f'<h2>{label} — eBay Preflight</h2><p><b>{"READY" if all_ok else "SAFETY STOP"}</b></p><p>SKU: <b>{sku}</b><br>Title: <b>{html.escape(title)}</b><br>Price: <b>${html.escape(price)}</b><br>Category: <b>{html.escape(cat_id)} — {html.escape(cat_name)}</b><br>Condition: <b>{html.escape(cond_enum)} / {html.escape(cond_id)}</b><br>Photos: <b>{len(items)}/24</b><br>Main photo: <b>source photo {main}</b><br>Photo order: <b>{html.escape(order_text)}</b><br>Shipping: <b>${html.escape(ship)}</b></p><p>Photo order is normalized by capture time / filename, not Google Picker return order.</p>{button}')
    except Exception as exc:
...
def ebay_current_setup():
    ...
    if not draft or request.form.get('shipping_verified')!='yes':
        return page(label+' setup','<h2>Safety stop — draft or shipping confirmation missing.</h2>'),400
    try:
        token=get_ebay_access_token()
        ...
        offer={'sku':sku,'marketplaceId':'EBAY_US','format':'FIXED_PRICE','availableQuantity':1,'listingDuration':'GTC','categoryId':draft['category_id'],'merchantLocationKey':LOT002_LOCATION_KEY,'listingDescription':desc,'listingPolicies':{'paymentPolicyId':LOT002_PAYMENT_POLICY,'returnPolicyId':LOT002_RETURN_POLICY,'fulfillmentPolicyId':pid},'pricingSummary':{'price':{'value':str(draft['price']),'currency':'USD'}}}
        try:
...
def next_lot():
    ...
    for key in ('picker_session_id','picker_uri','photo_editor_approved','main_photo_index','photo_order','photo_selection_signature'):
