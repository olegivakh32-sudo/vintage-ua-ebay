...
def _safe_photo_edit(image_bytes):
    """V14 TRUE-COLOR eBay edit: preserve the photographed item and condition.

    Only EXIF orientation, extremely gentle exposure correction, resizing and
    high-quality JPEG export are applied. No autocontrast, black-point crushing,
    background replacement, defect removal, or aggressive sharpening.
    """
    ...
    # V14: TRUE-COLOR / CONDITION-SAFE.  The previous autocontrast + contrast
    # boost could crush dark vintage surfaces into near-black and hide texture.
    # Keep tonal relationships essentially identical to the source photograph.
    ...
    # Intentionally NO ImageOps.autocontrast and NO contrast/colour boost.
    # A tiny sharpness lift compensates only for resize/JPEG softness.
    ...
    out = BytesIO()
    image.save(out, format="JPEG", quality=95, optimize=True, subsampling=0)
    return out.getvalue()
...
def _download_picker_photo(index, edited=False, max_side=2400):
    ...
    cache_key = (15, picker_id, index, max_side)
...
def picker_editor():
    ...
    for i in range(1, len(items) + 1):
        ...
        <div style='border:1px solid #ddd;border-radius:10px;padding:8px;margin-bottom:14px'>
          ...
          <div style='display:grid;grid-template-columns:1fr 1fr;gap:8px'>
            ...
            <div><b>Optimized for eBay</b><br><img class='edited-preview' src='/picker/edited/{i}?v=15' loading='eager' style='width:100%;max-height:280px;object-fit:contain;background:#f4f4f4'></div>
          </div>
        </div>""")
    ...
    return page("Current Lot Photo Editor", f"""
      ...
      <p>V15 TRUE-COLOR EDITOR: orientation + condition-safe exposure + eBay-ready JPEG. No autocontrast, black crushing, background cutting, defect removal or aggressive colour/contrast changes; texture, patina, scratches and ruler references are preserved.</p>
      {status}
      {''.join(cards)}
...
def picker_edited(index):
    try:
        data = _download_picker_photo(index, edited=True, max_side=1600)
...
def _auto_main_score(jpeg_bytes):
    """Deterministic fallback cover-photo score; AI selection is preferred in V15."""
    try:
...
def _choose_auto_main_photo(approved):
    """V15: choose the strongest truthful eBay cover image with vision; deterministic fallback."""
    if not approved:
        return 1
    try:
        content=[{"type":"input_text","text":"""Choose exactly ONE MAIN PHOTO for an eBay vintage/collectible listing from these seller-approved photos.
Return ONLY JSON: {\"main_photo\": N}.
Rules: choose the clearest complete overall view of the item; item should be large, centered and fully visible; prefer a natural buyer-friendly angle and clean background. Do NOT choose ruler/measurement photos, labels/marks, damage closeups, inside/detail-only views, extreme crops, duplicates, blurry/dark frames, or a photo where accessories dominate, unless every image is such a view. Never alter or infer the item. Photo numbers are supplied in order."""}]
        for i in sorted(approved):
            raw=approved[i]["bytes"]
            im=Image.open(BytesIO(raw)).convert("RGB")
            im.thumbnail((640,640), Image.Resampling.LANCZOS)
            buf=BytesIO(); im.save(buf,format="JPEG",quality=82,optimize=True)
        ...
        raw=call_openai({"model":"gpt-5.6-terra","input":[{"role":"user","content":content}],"reasoning":{"effort":"low"},"max_output_tokens":80,"store":False},timeout=120)
        m=re.search(r'\{[\s\S]*?\}',str(raw or ''))
        if m:
            choice=int(json.loads(m.group(0)).get("main_photo") or 0)
            if choice in approved:
                return choice
    except Exception:
        pass
    scored=[(_auto_main_score(approved[i]["bytes"]), i) for i in approved]
...
def picker_editor_approve():
    ...
    for i in range(1, len(items) + 1):
        ...
        for attempt in range(2):
            try:
                data = _download_picker_photo(i, edited=True, max_side=1600)
                if not data:
                ...
                PHOTO_EDIT_CACHE.pop((15, picker_id, i, 1600), None)
    ...
    if not (1 <= main_index <= len(items)):
        main_index=_choose_auto_main_photo(approved)
...
Rules: visual evidence overrides web similarity; never invent maker/model/year/country/material; distinguish maker marks from quality/state marks; clearly separate confirmed SOLD evidence from ACTIVE asking prices; use exact/near-exact confirmed SOLD first, then close sold analogs adjusted for condition/completeness; ACTIVE prices are only a secondary sanity check; never call an active or merely ended listing sold; do not average unrelated items; prefer recent sold evidence (roughly 90 days when available); compare total buyer cost including shipping; if fewer than 2 credible sold comps exist confidence cannot be high; if exact sold comps do not exist, derive a conservative BIN from closest analogs and label confidence. Optimize for eBay Best Match without keyword stuffing: identify high-intent truthful buyer search phrases, compare total buyer cost (item + shipping), and make Quick sale genuinely competitive for fast turnover.
...
- PRICE EVIDENCE HIERARCHY: exact/near-exact confirmed SOLD/COMPLETED first; then close sold analogs adjusted for condition/completeness; ACTIVE asking prices are only a secondary sanity check and must never drive the price when reliable sold evidence exists.
- Never call an active listing sold. Never treat an ended/removed listing as sold unless sale status is independently confirmed.
- Do not average unrelated items. Reject comps with materially different maker/model/function/size/material when closer evidence exists.
- For each useful sold comp, state why it is comparable and the important difference versus this item.
- Weight recent evidence more heavily; prefer roughly the last 90 days when available, then expand only when evidence is sparse.
- Price the seller's exact condition: working/tested status, defects, missing parts, completeness, restoration and visible wear can materially change value.
- Compare TOTAL BUYER COST (item price + shipping) where shipping is known.
- If fewer than 2 credible confirmed sold comps exist, do not present the estimate as high-confidence; use active/other evidence only as a conservative cross-check.
- Never reduce a price merely because eBay shows a generic percentage-above-similar-items banner; require item-level comparable evidence.
...
def ebay_current_preflight():
    ...
    try:
        ...
        all_ok=(1<=len(items)<=24 and bool(session.get('photo_editor_approved')) and 1<=main<=len(items) and len(order)==len(items) and bool(order) and order[0]==main and not offers and not missing_required and bool(draft.get('price_confirmed')) and bool(draft.get('seller_reviewed')))
        ...
        req_note=("<p style='color:#a05a00'><b>Required eBay specifics still missing:</b> "+html.escape(', '.join(missing_required))+"</p>") if missing_required else "<p style='color:#177a3f'><b>✓ Required eBay item specifics are complete.</b></p>"
        button=''
        if all_ok:
...
def ebay_current_setup():
    ...
    try:
        ...
        asp=_sanitize_identity_aspects(draft,asp)
        if missing_required:
            return page(label+' setup',f"<h2>Safety stop — required eBay item specifics are missing.</h2><p>{html.escape(', '.join(missing_required))}</p><p>Nothing was sent to eBay.</p>"),409
        draft['resolved_aspects']=asp; draft['missing_required_aspects']=[]; LOT002_DRAFT_CACHE[session.get('picker_session_id')]=draft
        ...
        inv={'availability':{'shipToLocationAvailability':{'quantity':ebay_qty}},'condition':draft['condition_enum'],'conditionDescription':(draft.get('condition_note') or '')[:1000],'product':product}
