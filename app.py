...
def picker_editor():
    ...
    status = "<p style='color:green'><b>✓ Edited photos are approved for eBay upload.</b></p>" if approved else "<p><b>Preview only.</b> eBay will keep using originals until you approve.</p>"
    return page("LOT 002 Photo Editor", f"""
      ...
      <form method='post' action='/picker/editor/approve'>
        <button type='submit' style='font-size:18px;padding:14px 18px'>Use edited {len(items)} photo(s) for eBay</button>
      </form>
...
def picker_items():
    ...
    if 1 <= count <= 24:
        ...
        button = """
        <p><a href="/picker/editor" style="display:inline-block;padding:12px 16px;border:1px solid #777;border-radius:7px;text-decoration:none">✨ Preview Auto Photo Editor</a></p>
        <form
...
def shipping():
    ...
    if not all([dim_l,dim_w,dim_h,actual_shipping_uah]):
        ...
        <p><b>Enter this item's own dimensions and actual Ukrposhta Ukraine → USA price.</b><br>LOT 001 clock data is intentionally not reused.</p>
        ...
        <p>Length, cm<br><input name="dim_l" type="number" step="0.1" min="0.1" required></p>
        <p>Width, cm<br><input name="dim_w" type="number" step="0.1" min="0.1" required></p>
        <p>Height, cm<br><input name="dim_h" type="number" step="0.1" min="0.1" required></p>
...
def final_listing():
    identification=request.form.get("identification",""); market=request.form.get("market",""); item_weight=request.form.get("item_weight",""); packed_weight=request.form.get("estimated_packed_weight",""); dimensions=request.form.get("dimensions",""); package_dimensions=request.form.get("package_dimensions",""); shipping_uah=request.form.get("actual_shipping_uah","")
    listing_prompt=f'''Create a final eBay.com listing draft in English for LOT 002.
Verified identification:\n{identification}\n\nMarket research:\n{market}
...
Return plain text only: 1 TITLE max 80 chars; 2 PRICE USD; 3 CONDITION; 4 ITEM SPECIFICS; 5 DESCRIPTION; 6 SEO KEYWORDS.
Do not invent model, origin, age, condition, completeness or specifications. Preserve defects and uncertainty. Use the Normal/recommended BIN price from market research exactly. Never reuse facts specific to LOT 001 clock. Draft only; do not claim publication.'''
    listing_response=call_openai({"model":"gpt-5.6-terra","input":listing_prompt,"reasoning":{"effort":"low"},"max_output_tokens":3500,"store":False},timeout=270)
