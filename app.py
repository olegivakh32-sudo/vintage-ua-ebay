...
def _white_background_edge_connected(image):
    ...
    if xs and ys:
        ...
        x0=max(0,x0-pad_x); x1=min(sw-1,x1+pad_x)
        y0=max(0,y0-pad_y); y1=min(sh-1,y1+pad_y)
        # Safety over cosmetic perfection: preserve everything in the item
        # envelope, even if that leaves some original pale background there.
