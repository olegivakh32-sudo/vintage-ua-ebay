...
@app.route("/picker/prepare-all", methods=["POST"])
def picker_prepare_all():
    """One-tap preparation of every active photo; no editor loop."""
    picker_id = session.get("picker_session_id")
    items = PHOTO_CACHE.get(picker_id, [])
    if not (1 <= len(items) <= 24):
        return page(
            "Photo preparation",
            "<h2>Preparation stopped: select from 1 to 24 active photos.</h2>"
            "<p><a href='/picker/start'>Select photos</a></p>",
        ), 400

    errors = []
    for index in range(1, len(items) + 1):
        try:
            _download_picker_photo(index, edited=True, max_side=2400)
        except Exception as exc:
            errors.append(f"Photo {index}: {exc}")

    if errors:
        session["photo_editor_approved"] = False
        detail = "<br>".join(html.escape(x) for x in errors)
        return page(
            "Photo preparation error",
            f"<h2>Some photos could not be prepared.</h2><p>{detail}</p>"
            "<p><a href='/picker/items'>Back to LOT 002 photos</a></p>",
        ), 502
...
def picker_items():
    ...
    if 1 <= count <= 24:
        ...
        if session.get("photo_editor_approved"):
            ...
            button = """
            <p><b>Required before analysis/eBay:</b> prepare the white-background versions.</p>
            <form action="/picker/prepare-all" method="post">
                <button type="submit" style="font-size:18px;padding:14px 18px;margin-top:8px">Prepare all 24 photos automatically</button>
            </form>
            <p style="font-size:13px;color:#555">One tap: all selected photos are processed, approved, and then you can continue to AI analysis.</p>
            """
    else:
        status = f"""
        <p style="color:red;">
