...
def picker_editor():
    ...
    items = PHOTO_CACHE.get(picker_id, [])
    if len(items) != 24:
        return page("Photo editor", "<h2>Select exactly 24 photos first.</h2><p><a href='/picker/start'>Select photos</a></p>"), 400
    ...
    return page("LOT 001 Photo Editor", f"""
      <h2>LOT 001 — Auto Photo Editor</h2>
      ...
      <form method='post' action='/picker/editor/approve'>
        <button type='submit' style='font-size:18px;padding:14px 18px'>Use edited 24 photos for eBay</button>
      </form>
...
def picker_editor_approve():
    ...
    items = PHOTO_CACHE.get(picker_id, [])
    if len(items) != 24:
        return page("Photo editor", "<h2>Approval stopped: 24 photos are not active.</h2>"), 400
...
<p>
    ...
    <p>
        <b>✓ Required shipping information is complete.</b><br>
        The calculated buyer shipping charge for this lot is
        <b>${ebay_shipping_usd:.2f} USD</b>.
    </p>
