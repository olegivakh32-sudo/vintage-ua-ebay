...
def _market_bin_price(market_text):
    """Choose the researched BIN according to the configured sales strategy.

    FAST (default): use the model's Quick sale price to favor conversion/turnover.
    BALANCED: use Normal Buy It Now.
    MARGIN: use Patient seller price.
    Falls back conservatively if the requested tier was not parsed.
    """
    ...
    mode = str(globals().get("EBAY_SALES_MODE") or "FAST").upper()
    if mode == "MARGIN" and patient:
        return patient
    if mode == "BALANCED" and normal:
        return normal
    if mode == "FAST" and quick:
        return quick
    if normal:
        return normal
    if quick:
...
def fast_research():
    ...
    content = [{"type":"input_text","text":f"""Research ONE vintage/collectible item for an eBay.com seller. Current lot: {label}.
